...
void main() {
  ...
  Future<ProviderContainer> buildContainer(
    ...
    await container.read(coachConversationsProvider.notifier).ensureActiveConversation();
    await container.read(coachChatProvider.future);
    return container;
  }

  test('resolves a TOOL_CALL directive before answering, and shows the step', () async {
    ...
    await container.read(coachChatProvider.notifier).sendMessage(
  ...
  test('answers directly and streams live when no tool call is made', () async {
    ...
    await container.read(coachChatProvider.notifier).sendMessage('Should I run today?');
  ...
  test('stops looping after the max tool step budget and answers', () async {
    ...
    await container.read(coachChatProvider.notifier).sendMessage('Give me a full report.');

    final messages = container.read(coachChatProvider).value!;
    final assistant = messages.last;

    // maxToolStepsPerTurn = 2, so exactly 2 tool calls run (steps 0 and 1)
    // before the 3rd call is treated as the final answer regardless.
    expect(assistant.toolSteps, hasLength(2));
    expect(assistant.hasError, isFalse);
